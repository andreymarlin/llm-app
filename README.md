...
| `empty reply from server` при скачивании модели | Сервер Hugging Face оборвал соединение. `setup.ps1` сам повторяет загрузку (до 10 попыток) и докачивает с места обрыва. Если повторяется постоянно — отключите VPN/прокси и антивирус, проверьте интернет |
